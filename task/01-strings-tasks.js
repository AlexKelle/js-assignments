...
function removeFirstOccurrences(str, value) {
    return str.slice(0,str.indexOf(value)) + str.substring(str.indexOf(value)+ value.length,str.length);
}
...
function getRectangleString(width, height) {
    ...
    for (let i  = 0; i < height; i++) {
      ...
      stringChars.push(firstChar);
      for (let j = 1; j < width - 1; j++) {
        stringChars.push(middleChar);
      }
      stringChars.push(lastChar);
      stringChars.push('\n');

      result = result + stringChars.join('');
    }
